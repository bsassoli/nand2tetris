...
class Compiler:
    ...
    def compileClass(self):
        ...
        while self.current_token in ["constructor", "method", "function"]:
    ...
    def compileLet(self):
        ...
        while self.current_token != ";":         
            ...
            self.compileExpression()
        # semicolon
        print("YEYOOOOOOOOOOOOOOOOOOOOOOO")
    ...
    def compileIf(self):
        ...
        assert (
            self.get_current_token_tags()[1] == "if"
        ), "If statment must begine with if"
        self.out += self.current_tagged_token
        self.advance()
        assert (
            self.get_current_token_tags()[1] == "("
        ), "If statment must be followed by left paren"
        self.out += self.current_tagged_token
        self.advance()
        self.compileExpression()
        self.out += self.current_tagged_token
        ...
        print(
            f"Breaking out of if Statement at position: {self.current_position} with current token: {self.current_token}"
        )
        self.out += "</ifStatement>\n"
    ...
    def compileSubroutineCall(self):
        ...
        self.out += self.current_tagged_token
        self.advance()
        print("HEY YOU!!!!!!!! I AM OUT!")
        # Either expression list
        if self.current_token == "(":
            print("HEY YOU!!!!!!!! I AM IN!")
            print("left paren before exp list " + self.current_token)
            # left paren
            self.out += self.current_tagged_token
            # self.advance()
            print("HEY YOU!!!!!!!! I ADVANCED! " + self.current_token)
            # expression list
        ...
        # Or ( className | varName) '.' subroutineName '(' expressionList ')'
            ...
            self.out += self.current_tagged_token
            self.advance()
            print("Printing l paren: " + self.current_token)
            # expressionList
            self.compileExpressionList()
            # print ')'
            self.out += self.current_tagged_token
